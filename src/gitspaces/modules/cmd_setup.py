...
def run_setup() -> bool:
    ...
    while True:
        path = Console.prompt_input(
            "Enter a project directory path (or press Enter to finish):",
            default=""
        )
        ...
        if not expanded_path.exists():
            create = Console.prompt_confirm(
                f"Directory {expanded_path} does not exist. Create it?",
                default=True
            )
            if create:
    ...
    editor = Console.prompt_input(
        "Enter your preferred editor command (e.g., 'code', 'vim'):",
        default="code"
    )
